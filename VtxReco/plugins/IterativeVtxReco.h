  ...
  private:
    ...
    template <typename T>
      void print_track_set(const T & ts, const reco::Vertex & v) const {
        for (auto r : ts)
          printf(" %u%s", r.key(), (v.trackWeight(r) < track_vertex_weight_min ? "!" : ""));
      }

    void print_track_set(const reco::Vertex & v) const {
      for (auto r = v.tracks_begin(), re = v.tracks_end(); r != re; ++r)
        printf(" %lu%s", r->key(), (v.trackWeight(*r) < track_vertex_weight_min ? "!" : ""));
    }
    ...
    track_set vertex_track_set(const reco::Vertex & v, const double min_weight = track_vertex_weight_min) const {
      track_set result;
    ...
    track_vec vertex_track_vec(const reco::Vertex & v, const double min_weight = track_vertex_weight_min) const {
